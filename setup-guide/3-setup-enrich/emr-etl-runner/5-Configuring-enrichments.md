...
[**HOME**](Home) > [**SNOWPLOW SETUP GUIDE**](Setting-up-Snowplow) > [Step 3: Setting up Enrich](Setting-up-enrich) > Configuring enrichments
  ...
  - 4. [Individual enrichments](#enrichments)
    ...
    - 4.5 [user_agent_utils_config](#user_agent_utils_config)
...
<a name="user_agent_utils_config"/>
### 4.5 user_agent_utils_config enrichment
...
<a name="ua_parser_config"/>
### 4.6 ua_parser_config enrichment

This enrichment uses the [ua-parser][ua-parser] library to parse the useragent and attach a ua_parser_context to each event. It can be used alongside or instead of the user_agent_utils_config enrichment. Enable it with the following JSON:
...
<a name="currency_conversion_config"/>
### 4.7 currency_conversion_config enrichment
...
Its JSON schema can be found [here][currency_schema].
...
[currency_schema]: http://iglucentral.com/schemas/com.snowplowanalytics.snowplow/currency_conversion_config/jsonschema/1-0-1
